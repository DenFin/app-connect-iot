<template>
  <div>
    <div class="md:flex gap-5">
      ...
      <span
        class="align-middle xl:text-4xl text-2xl font-thin xl:pt-5 md:pt-3 opacity-70">{{ store.isTestData? testSystems.systems.items.length : systems?.systems?.items?.length }} {{ $t('available') }}</span>
    </div>
      ...
      <div v-for="system in store.isTestData ? testSystems.systems.items : systems?.systems?.items" :key="system.id">
        <BegaCard :title="system.name">
          <template #preHeadline>
            <span class="text-xs uppercase">{{system?.connectors[0].gears_count}} {{$t('lamps')}}</span>
          </template>
          <template #content>
            <div v-if="system.location && system.location.address" class="mb-6">{{ system.location.address }}</div>
            <BegaLink data-testid="system-google-maps-link" v-if="system.location && system.location.address" link-color="text-secondary"
            ...
            <BegaButtonATag class="mt-6 shadow-none hover:shadow-none border border-primary" :is-primary="false" text-classes="font-semibold" show-arrow
